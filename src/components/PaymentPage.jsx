...
const Card = ({ amount, setCart, setTotalPrice, cart }) => {
  ...
  function payment(e) {
    ...
    if (userName !== name) {
      return toast.error("იუზერის სახელი არასწორია");
    }
    ...
    if (user.balance < Number(amount)) {
      ...
      if (
        ...
        expiryD == user.expiryD &&
        expiryM == user.expiryM &&
        cvc == user.CVC
      ) {
        ...
        const newPaymentHistory = {
          ...
          totalPrice: amount,
          date:
            now.getHours() + ":" + now.getMinutes() + "\t" + now.toDateString(),
        };
        ...
        if (checked == true) {
          localStorage.setItem("card", JSON.stringify(cardOjb));
        }
      } else {
  ...
  return (
    <div className="flex justify-center items-center w-full h-screen bg-mauve-900 ">
      <div className="w-220 h-160 bg-mauve-700 relative text-white rounded-2xl">
        <h1 className="text-5xl font-bold text-white absolute top-5 left-15">
          Payment Page
        </h1>
        <div className="w-160 h-110 flex justify-center items-center mt-25 ml-15 p-10 bg-mauve-900 shadow-2xl shadow-mauve-950 rounded-2xl">
          ...
          <form
            ...
            className="w-3/8 h-full flex flex-col gap-5"
          >
            ...
            <div className="flex flex-col gap-7">
              ...
              <button
                type="submit"
                className="w-fit h-fit px-5 py-2 rounded-lg bg-white text-md font-extrabold text-mauve-900 shadow-lg shadow-amber-50/20 hover:cursor-pointer hover:scale-105"
              >
                Pay now
              </button>
            </div>
